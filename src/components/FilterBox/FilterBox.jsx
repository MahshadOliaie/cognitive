...
function FilterBox({ filterList, header , setIsFilterOpen, columnFilters, setColumnFilters }) {

    const filterItems = columnFilters.find(f => f.id === header.id)?.value || []
        ...
        <div className="absolute" style={{ right: "30%" }}>
            <div className="bg-sand mt-4 rounded-lg flex flex-col w-full text-right h-60 gap-1 overflow-scroll shadow-xl" >
                ...
                {newFilterList.map((item, index) => {
                    return <FilterItem item={item} id={header.id} key={index} columnFilters={columnFilters} setColumnFilters={setColumnFilters} isActive={filterItems.includes(item)} />
                })}
            </div>
